...
function num(x){ if(x==null) return 0; let s=String(x).trim().replace(/[^\d.,-]/g,""); if(s.includes(",")&&s.includes(".")){ if(s.lastIndexOf(".")>s.lastIndexOf(",")) s=s.replace(/,/g,""); else s=s.replace(/\./g,"").replace(",","."); } else if(s.includes(",")&&!s.includes(".")) s=s.replace(",","."); else s=s.replace(/,/g,""); const n=Number(s); return isNaN(n)?0:n;}
...
function aggregate(all){
  ...
  // Top clients by value (A/B) and by count of invoices (A/B) — count by unique invoice per phone.
  const valA={}, valB={}, cntA={}, cntB={}; // value and counts
  const seenInvA=new Map(), seenInvB=new Map(); // phone -> Set(invoices)
  all.forEach(r=>{
    if(!r.client) return;
    if(r.type==="A"){
      valA[r.client]=(valA[r.client]||0)+r.line;
      if(r.phone){
        if(!seenInvA.has(r.phone)) seenInvA.set(r.phone,new Set());
        seenInvA.get(r.phone).add(r.invoice);
      }
    }
    if(r.type==="B"){
      valB[r.client]=(valB[r.client]||0)+r.line;
      if(r.phone){
        if(!seenInvB.has(r.phone)) seenInvB.set(r.phone,new Set());
        seenInvB.get(r.phone).add(r.invoice);
      }
    }
  });
  // Convert phone-based counts back to client names approximating by sum across rows with that phone
  // We'll make a phone->client map using first occurrence
  const phoneToClientA=new Map(), phoneToClientB=new Map();
  all.forEach(r=>{
    if(r.type==="A" && r.phone && !phoneToClientA.has(r.phone)) phoneToClientA.set(r.phone, r.client||r.phone);
    if(r.type==="B" && r.phone && !phoneToClientB.has(r.phone)) phoneToClientB.set(r.phone, r.client||r.phone);
  });
  for(const [ph,set] of seenInvA.entries()){
    const name = phoneToClientA.get(ph) || ph;
    cntA[name]=(cntA[name]||0)+set.size;
  }
  for(const [ph,set] of seenInvB.entries()){
    const name = phoneToClientB.get(ph) || ph;
    cntB[name]=(cntB[name]||0)+set.size;
  }
  ...
  return { invoices:invoices, turnover, totalQty, avg,
           topA_value, topB_value, topA_count, topB_count, bestVal, bestQty };
}

function buildFilters(all){
  const cats=uniqSorted(all.map(r=>r.category)), subs=uniqSorted(all.map(r=>r.subcat));
  const catSel=document.getElementById("filter-category"), subSel=document.getElementById("filter-subcat");
...
function setPillset(pillsetEl, mode){
  pillsetEl.querySelectorAll(".pill").forEach(b=>b.setAttribute("aria-pressed", b.dataset.mode===mode ? "true":"false"));
}

function renderTopClients(el, list, mode){
  // list: array of [name, value/count]; mode controls right-hand label
  el.innerHTML="";
  list.slice(0,20).forEach(([name,amt],i)=>{
    const li=document.createElement("li");
    li.className="li";
    li.innerHTML=`<div class="grow">
        <div class="name">${i+1}. ${name||"Unknown"}</div>
      </div>
...
async function main(){
  try{
    ...
    function recomputeAndRender(){
      ...
      // Top clients: mode toggle per column
      ...
      const listA = state.clientsA==="value" ? agg.topA_value : agg.topA_count;
      const listB = state.clientsB==="value" ? agg.topB_value : agg.topB_count;
      renderTopClients(document.getElementById("list-topA"), listA, state.clientsA);
      renderTopClients(document.getElementById("list-topB"), listB, state.clientsB);

      // Best items
      setPillset(document.getElementById("itemsMode"), state.items);
      const items = state.items==="value" ? agg.bestVal : agg.bestQty;
      renderBestItems(document.getElementById("list-items"), items, state.items);
    }
